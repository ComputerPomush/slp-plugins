	...
	class SLP_Extended_Data_Manager_Admin_General extends SLPlus_BaseClass_Object {
        ...
        public function add_data_element_subtab( $settings ) {
			...
			if ( slp_edm_freemius()->is__premium_only() ) {
				$this->element_manager_add->render_ElementAddGroup( $settings );
			}
		...
		function createobject_ElementManagers() {
			...
			if ( slp_edm_freemius()->is__premium_only() ) {
				if ( !isset( $this->element_manager_add ) ) {
					$this->element_manager_add = new SLP_Extended_Data_Manager_AdminUI_ElementManager_Add(
						array(
							'addon'     => $this->addon,
							'admin'     => $this->admin,
							'slplus'    => $this->slplus,
			...
			if ( !isset( $this->element_manager_table ) ) {
				$this->element_manager_table = new SLP_Extended_Data_Manager_AdminUI_ElementManager_Table(
					array(
						'addon'     => $this->addon,
						'admin'     => $this->admin,
						'slplus'    => $this->slplus,
        ...
        private function add_manage_extended_data_elements_group( $settings ) {
            $section_params['name'] = __( 'Data' , 'slp-extended-data-manager' );  // with the add_extended_data_element_group method called first we don't really need this.
            $section_params['slug'] = SLP_EDM_SECTION_SLUG;
            $settings->add_section( $section_params );

            $group_params['header']         = __('Extended Data Elements','slp-extended-data-manager');
			if ( slp_edm_freemius()->is__premium_only() ) {
				$group_params['header']     = __('Manage Extended Data Elements','slp-extended-data-manager');
			}
			...
			if ( slp_edm_freemius()->is__premium_only() ) {
				$settings->add_ItemToGroup(array(
					...
					'group_slug'    => SLP_EDM_SECTION_SLUG_TABLE,
					'show_label'    => false,
					'value'         => __('Update Elements','slp-extended-data-manager'),
					'type'          => 'submit_button',
					'description'   => ''
				));
			}
