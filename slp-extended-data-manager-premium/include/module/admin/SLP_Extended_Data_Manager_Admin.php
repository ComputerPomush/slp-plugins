...
if (! class_exists('SLP_Extended_Data_Manager_Admin')) {
	...
	class SLP_Extended_Data_Manager_Admin extends SLP_BaseClass_Admin {
		...
		public function add_hooks_and_filters() {
			...
			if ( slp_edm_freemius()->is__premium_only() ) {

				// Manage Locations UI
				//
				add_filter('slp_locations_manage_filters'           ,array($this,'filter_locations_manage_filters_EDM__premium_only'              ), 110      );
				add_action('slp_manage_locations_action'            ,array($this,'action_manage_locations_action_EDM__premium_only'               )           );

				// SLP Filters
				//
				add_filter('slp_manage_location_columns'            ,array($this,'filter_slp_manage_location_columns_EDM__premium_only'           ), 999      );
			}
		...
		private function create_object_admin_objects() {
			...
			$this->edm_admin_elements = new SLP_Extended_Data_Manager_Admin_Elements( array( 'addon' => $this->addon, 'admin' => $this ) );
		...
		private function create_object_info() {
			...
			if ( ! isset( $this->info ) ) {
				...
				$this->info = new SLP_Extended_Data_Manager_Admin_Info( array( 'addon' => $this->addon, 'admin' => $this ) );
			}
		}
        ...
        private function create_object_general() {
            ...
            if ( ! isset( $this->edm_admin_general ) ) {
                ...
                $this->edm_admin_general = new SLP_Extended_Data_Manager_Admin_General(array( 'addon' => $this->addon, 'admin' => $this ) );
            }
        }
		...
		function create_object_edm_activation() {
			...
			if ( ! isset( $this->edm_activation ) ) {
				$this->edm_activation = new SLP_Extended_Data_Manager_Activation( array( 'addon' => $this->addon, 'admin' => $this ) );
        ...
        function action_SaveGeneralSettingsEDM() {
			...
			if ( slp_edm_freemius()->is__premium_only() ) {
				...
				// Check and process SLP_EDM_ACTION_ELEMENT_ADD with priority
				if (isset($_REQUEST[SLP_EDM_ACTION_ELEMENT_ADD])) {
					$edm_new_slug = sanitize_key( $_REQUEST[SLP_EDM_ACTION_ELEMENT_ADD] );
					if (!empty($edm_new_slug)) {
						$this->debugMP('msg',__FUNCTION__.' ADD_FIELD for edm_new_slug ' . $edm_new_slug);

						// Add the new element_slug to the extended data set
						$this->slplus->database->extension->add_field( $edm_new_slug, 'varchar', array( 'addon' => $this->addon->short_slug ), 'immediate');
						return;
					}
				}
			}
...
//					$this->debugMP('pr',__FUNCTION__.' started for intValue = ' . is_numeric($curValue) . ' for curValue: ', $curValue);
					if (isset($table_data[$curSlug])) {
						$this->edm_admin_elements->slp_edm_element_update($curSlug, $curValue, SLP_EDM_OPTION_ORDER, $intValue);
					}
				}
		...
		function filter_AddOurAdminSlug($slugs) {
			...
			$slugs = array_merge($slugs,
					array(
						SLP_EDM_ADMIN_PAGE_SLUG,
						SLP_ADMIN_PAGEPRE . SLP_EDM_ADMIN_PAGE_SLUG,
						)
					);
	    ...
	    function add_meta_links( $links, $file ) {
		    ...
		    if ( $file == $this->addon->slug ) {
			    ...
			    $links[] = sprintf( '<a href="%s" title="%s" target="store_locator_plus">%s</a>' , SLP_EDM_SUPPORT_URL , $link_text, $link_text);

				// Add Settings link
		    	$link_text = __( 'Settings', 'slp-extended-data-manager' );
			    $links[] = sprintf( '<a href="%s" title="%s">%s</a>' , admin_url( 'admin.php?page=' . SLP_EDM_ADMIN_PAGE_SLUG ) , $link_text, $link_text);
		...
		/**
		 * Add more options to the Filter drop down on the admin Locations/Manage Locations interface.
		 *
		 * @param mixed[] $dropdownItems
		 *
		 * @return mixed[]
		 */
		function filter_locations_manage_filters_EDM__premium_only( $dropdownItems ) {
			//$this->debugMP('pr',__FUNCTION__ . ' dropdownItems = ', $dropdownItems);

			// Get some valuable parameters
			$itemLabel = __('Show Hidden EDM Columns','slp-extended-data-manager');

			$newDropdownItems = array_merge($dropdownItems,
					array (
						array(	'label'     => $itemLabel,
								'value'     => SLP_EDM_FILTER_ACTION,
								'selected'  => $this->is_CurrentAction_edm_filter_action_selected(),
						),
					)
			);
			//$this->debugMP('pr',__FUNCTION__ . ' currentDisplayMode = ' . $currentDisplayMode . ', newDropdownItems = ', $newDropdownItems);

			return $newDropdownItems;
		}

		/**
		 * Handle the option SLP_EDM_FILTER_ACTION when selected
		 *
		 * @return boolean true when success
		 */
		function action_manage_locations_action_EDM__premium_only() {

			// Set the valuable parameter
			$this->debugMP('msg',__FUNCTION__ . ' is_CurrentAction_edm_filter_action_selected = ' . $this->is_CurrentAction_edm_filter_action_selected());
		...
		/**
		 * Handle the display of the option SLP_EDM_FILTER_ACTION when selected
		 *
		 * SLP Filter: slp_manage_location_columns
		 *
		 * @param mixed[] $currentCols column name + column label for existing items
		 * @return mixed[] column name + column labels, filtered according to the EDM settings
		 */
		function filter_slp_manage_location_columns_EDM__premium_only($currentCols) {
			$this->debugMP('pr',__FUNCTION__ . ' currentCols (#' . count($currentCols) . ') = ', $currentCols);

			$this->action_manage_locations_action_EDM__premium_only();

			// Get the data to process
			$edmCols  = $currentCols;
			$numUnset = 0;
			$showHiddenColumns = $this->is_CurrentAction_edm_filter_action_selected();

			// Prepare the data for processing
			$colData     = $this->slplus->database->extension->get_cols(true);
			$optionShow  = SLP_EDM_OPTION_SHOW;
			$optionOrder = SLP_EDM_OPTION_ORDER;
			$sortCols = array();
			$tempSortOrder = - count($currentCols); // Used to keep original order for non-External Data Elements
			foreach ($currentCols as $key => $value) {
				$sortCols[$key] = $tempSortOrder++;
			}

			// Filter the Extended Data Elements found
			$tempSortOrder = SLP_EDM_OPTION_ORDER_MAX + 10; // Used to keep original order for non-ordered External Data Elements
			foreach ($colData as $key => $curCol) {
				if (isset($edmCols[$curCol->slug])) {
					$col_options = maybe_unserialize($curCol->options);
					$colShow     = (isset($col_options[$optionShow])  ? $col_options[$optionShow]  : true);
					$colOrder    = (isset($col_options[$optionOrder]) ? $col_options[$optionOrder] : '');
					if (($showHiddenColumns) || ($this->slplus->is_CheckTrue($colShow))) {
						$sortCols[$curCol->slug] = ($colOrder == '' ? $tempSortOrder++ : $colOrder);
					} else {
						unset($edmCols[$curCol->slug]);
						unset($sortCols[$curCol->slug]);
						$numUnset++;
					}
				}
			}

			// Order the remaining Extended Data Elements
			asort($sortCols, SORT_NUMERIC);
			$returnCols = array();
			foreach ($sortCols as $key => $value) {
				$returnCols[$key] = $currentCols[$key];
			}

			$this->debugMP('pr',__FUNCTION__ . ' is_CurrentAction_edm_filter_action_selected = ' . $this->is_CurrentAction_edm_filter_action_selected() . ', returnCols (E' . count($returnCols) . ', U:' . $numUnset . ') = ', $returnCols);
		...
		public function get_sanitize_key_array( $input_array = null, $input_key = '' ) {
			...
			if ( $input_array == null )           { return ''; }
			if ( $input_key   == '')              { return ''; }
			if (!isset($input_array[$input_key])) { return ''; }
