...
class SLP_Extended_Data_Manager_AdminUI_ElementManager_Table extends WP_List_Table {
	...
	public function initialize() {
		...
		if (isset($_SERVER['REQUEST_URI'])) {
			...
			//
			if (isset($_REQUEST['page'])) { $queryParams['page'] = esc_url($_REQUEST['page']); }
...
//			SLP_EDM_TABLE_OPTIONS      => __( 'Options',      'slp-extended-data-manager' ),
	...
	function get_sortable_columns() {
		...
		$sortable_columns = array(
			SLP_EDM_TABLE_FIELD_ID      => array(SLP_EDM_TABLE_FIELD_ID,true),     //true means it's already sorted
...
//			SLP_EDM_TABLE_OPTIONS       => array(SLP_EDM_TABLE_OPTIONS,false),
	...
	function column_cb($item){
		return sprintf(
			...
			/*$1%s*/ $this->_args['singular'],  //Let's simply repurpose the table's singular label ("edm_slug")
			/*$2%s*/ $item[SLP_EDM_TABLE_SLUG]         //The value of the checkbox should be the record's id
			);
	}
	...
	function column_option_order($item){
		...
		return sprintf(
			...
			/*$1%s*/ $request_name_option,     //The key should reflect the option name
			/*$2%s*/ $item[SLP_EDM_TABLE_ORDER],
			/*$3%s*/ SLP_EDM_OPTION_ORDER_MAX
			);
	}
	...
	function column_option_help_text($item){
		...
		return sprintf(
			...
			/*$1%s*/ $request_name_option,     //The key should reflect the option name
			/*$2%s*/ $num_rows,
			/*$3%s*/ $item[SLP_EDM_TABLE_HELP_TEXT]
			);
	}
	...
	function column_label($item){
		...
		if ( slp_edm_freemius()->is__premium_only() ) {
			...
			$row_pattern = '<a href="?page=%s&%s=%s&%s=%s">%s</a>';

			//Build row actions
			$actions['delete'] = sprintf( $row_pattern, $_REQUEST['page'], SLP_EDM_ACTION, SLP_EDM_ACTION_ELEMENT_DELETE, $this->_args['singular'], $item[SLP_EDM_TABLE_SLUG], __( 'Delete','slp-extended-data-manager' ) );
			$actions['show']   = sprintf( $row_pattern, $_REQUEST['page'], SLP_EDM_ACTION, SLP_EDM_ACTION_ELEMENT_SHOW,   $this->_args['singular'], $item[SLP_EDM_TABLE_SLUG], __( 'Show',  'slp-extended-data-manager' ) );
			$actions['hide']   = sprintf( $row_pattern, $_REQUEST['page'], SLP_EDM_ACTION, SLP_EDM_ACTION_ELEMENT_HIDE,   $this->_args['singular'], $item[SLP_EDM_TABLE_SLUG], __( 'Hide',  'slp-extended-data-manager' ) );
	...
	function get_bulk_actions() {
		...
		if ( slp_edm_freemius()->is__premium_only() ) {
			$actions[ SLP_EDM_ACTION_ELEMENT_DELETE ] = __( 'Delete Elements','slp-extended-data-manager' );
			$actions[ SLP_EDM_ACTION_ELEMENT_SHOW   ] = __( 'Show Elements',  'slp-extended-data-manager' );
			$actions[ SLP_EDM_ACTION_ELEMENT_HIDE   ] = __( 'Hide Elements',  'slp-extended-data-manager' );
		}
		return $actions;
	}
	...
	function createstring_FiltersBlock() {
		...
		$dialog_options = 
			...
			"position: { my: 'left top', at: 'left bottom', of: '#filterTypeEDM' } "
			;
	...
	function process_bulk_action() {
		...
		if ( slp_edm_freemius()->is__premium_only() ) {
			...
			if (!isset($_REQUEST['edm_slug'])) { return false; }
			$slugValues = array();
			if (is_array($_REQUEST['edm_slug'])) {
				foreach( $_REQUEST['edm_slug'] as $slugValue ) {
					$slugValues[] = sanitize_key($slugValue);
				}
			} else {
				$slugValues[] = sanitize_key($_REQUEST['edm_slug']);
			}
			$this->debugMP('pr',__FUNCTION__ . ' slugValues:',$slugValues);

			//Detect when and which bulk action is being triggered...
			switch ($this->current_action()) {

				case SLP_EDM_ACTION_ELEMENT_DELETE:
					$this->debugMP('pr',__FUNCTION__.' Delete extended data elements:',$slugValues);
					$table_data = $this->edm_admin_elements->get_extended_data_elements( false );
					foreach ($slugValues as $curSlug) {
						if (isset($table_data[$curSlug])) {
							$this->edm_admin_elements->slp_edm_element_delete($curSlug,$table_data[$curSlug]);
						}
					}

					// Set the selected_nav_element
					$this->admin->set_selected_nav_element();
					//$_REQUEST['selected_nav_element'] = SLP_EDM_SELECTED_NAV_ELEMENT;

					break;

				case SLP_EDM_ACTION_ELEMENT_SHOW:
					$this->debugMP('pr',__FUNCTION__.' Show extended data elements:',$slugValues);
					foreach ($slugValues as $curSlug) {
						$this->edm_admin_elements->slp_edm_element_show_hide($curSlug, 'true');
					}
				...
				case SLP_EDM_ACTION_ELEMENT_HIDE:
					$this->debugMP('pr',__FUNCTION__.' Hide extended data elements:',$slugValues);
					foreach ($slugValues as $curSlug) {
						$this->edm_admin_elements->slp_edm_element_show_hide($curSlug, 'false');
					}
	...
	function sort_data_elements($data_elements = '') {
		...
		if ( ! function_exists( 'usort_table_order' ) ) {
			function usort_table_order( $a, $b ) {
				$orderby = ( ! empty( $_REQUEST['orderby'] ) ) ? sanitize_sql_orderby($_REQUEST['orderby']) : SLP_EDM_TABLE_SLUG; //If no sort,  default to tab_slug
				$order   = ( ! empty( $_REQUEST['order'] ) )   ? sanitize_text_field( $_REQUEST['order']  ) : 'asc';              //If no order, default to asc
				if ( $orderby == SLP_EDM_TABLE_ORDER ) {
					$result = intval( $a[ $orderby ] ) - intval( $b[ $orderby ] ); //Determine sort order for integers
				} else {
