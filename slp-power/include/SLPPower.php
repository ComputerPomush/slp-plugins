...
class SLPPower extends SLP_BaseClass_Addon {
	...
	public $admin;
	private $cache_icon_strings = array();
	...
	public static function init() {
		...
		if ( ! $instance ) {
			...
			$instance = SLPPower::get_instance( false, array(
			...
			add_action( 'widgets_init', array( $instance, 'widgets_init' ) );
		}
	...
	public function widgets_init() {
		register_sidebar( array(
			'name'          => esc_html__( 'No Store Pages', 'slp-power' ),
			'id'            => 'slp-power-no-pages',
			'description'   => esc_html__( 'These widgets are shown when an [slp_pages] directory returns no locations.', 'slp-power' ),
	 ...
	 */
	public function create_CSVLocationImporter() {
	 ...
	 *
	 * @uses    \SLPPower::createstring_CategoryIconHTML
	 *
	 * @used-by \SLP_Power_Category_Walker_Legend::start_el
	 * @used-by \SLPPower::create_string_icon_array
	 */
	public function create_LocationIcons( $categories, $params = array() ) {
	 ...
	 */
	private function createstring_CategoryEditLink( $category_id, $html ) {
	 ...
	 *
	 * @used-by \SLPPower::create_LocationIcons
	 *
	 * @uses    \SLPPower::createstring_CategoryImageHTML
	 * @uses    \SLPPower::createstring_CategoryLegendText with show_label param set
	 * @uses    \SLPPower::createstring_CategoryEditLink with add_edit_link param set
	 */
	private function createstring_CategoryIconHTML( $category, $params ) {
	...
	public function create_string_icon_array( $params = array() ) {
		$params = array_merge( array( 'add_edit_link' => false, 'show_label' => false ), $params );

		$so = SLP_SmartOptions::get_instance();
		if ( ! is_a( $so, 'SLP_SmartOptions' ) ) {
			return '';
		}
		if ( ! property_exists( $so, 'show_icon_array' ) ) {
			return '';
		}
		// Short circuit if not an edit link and show_icon_array is off (for FE UI)
		if ( ! $params['add_edit_link'] && $so->show_icon_array->is_false ) {
			return '';
		}
		...
		if ( count( $this->current_location_categories ) > 0 ) {
			$key = join( '.', $this->current_location_categories );

			// Check our memory-based icon string cache, if empty create it.
			//
			if ( empty( $this->cache_icon_strings[ $key ] ) ) {
				...
				$this->cache_icon_strings[ $key ] = $icon_string;

				// Cache hit for icon string, use that.
			} else {
				$icon_string = $this->cache_icon_strings[ $key ];
			}
	...
	public function filter_AddMenuItems( $menuItems ) {
		...
		if ( $this->slplus->SmartOptions->reporting_enabled->is_true ) {
			$this->admin_menu_entries[] = array(
				...
				'slug'     => 'slp_reports_2022',
				'class'    => $this->admin->reports2022_tab,
				'function' => 'render',
			);
		}
		$this->admin_menu_entries[] = array(
	...
	public function recode_all_uncoded_locations( $messages = null, $max_id = null ) {
		...
		do {
			...
			if ( ! empty ( $location['sl_id'] ) ) {
				...
				if ( $this->slplus->currentLocation->MakePersistentIfChanged() ) {
					if ( ! is_null( $max_id ) ) {
	 ...
	 */
	public function set_LocationCategories() {
		if ( $this->categories_loaded_for == $this->slplus->currentLocation->id ) {
			return;
		}
		$this->current_location_categories = $this->slplus->database->get_Record( 'select_categories_for_location', $this->slplus->currentLocation->id, 0, ARRAY_A, 'get_col' );
		if ( empty( $this->current_location_categories ) ) {
