...
class SLP_Power_Category_Manager extends SLPlus_BaseClass_Object {
	...
	/**
	 * @var WP_Term[] $wp_categories
	 */
	public $wp_categories;

	/**
	 * @var int $wp_category_count Count of active categories. Fetch with get_category_count()
	 */
	private $wp_category_count;

	/**
	 * @var string[] $category_names The category names, index is the term ID.
	 */
	private $category_names;

	/**
	 * @var SLP_Power_Category_Data $category_data The category table database interface.
	 */
	public $category_data;

	/**
	 * @var int[] $location_categories The current location category ID array.
	 *
	 ...
	 *
	 * @used-by \SLP_Power_Locations_Import::prepare_for_import via filter 'create_categories_from_location_data'
	 *
	 * @return string[]
	 */
	 ...
	 *
	 * @used-by \SLP_Power_Category_Manager::create_categories_from_location_data
	 *
	 * @return null
	 */
	private function create_store_category( $categoryData ) {
		...
		$category_name_list      = explode( '::', $categoryData['category'] );
	...
	private function delete_detached_categories() {
		...
		$this->category_data->db->query(
			$this->category_data->db->prepare(
				$this->category_data->get_SQL( 'delete_category_by_id' ), $this->slplus->currentLocation->id ) .
	...
	private function get_all_categories() {
		if ( ! isset( $this->wp_categories ) ) {
			...
			$this->wp_categories = get_categories( array(
	 ...
	 *
	 * This builds the HTML of <div><ul><li>... which contains the checklist input elements, it is not the functional checked/unchecked just the DOM interface for it.
	 *
	 * @used-by \SLP_Power_Admin_Location_Filters::createstring_LocationFilterForm -- Admin location filters (non-Vue)
	 * @used-by \SLP_Power_Admin_Locations::extend_bulk_actions -- Bulk categorization modal (non-Vue)
	 * @used-by \SLP_Power_Admin_Locations::modify_add_form -- Admin adding/editing locations (Vue)
	 *
	...
	public function get_check_list( $post_id ) {
		...
		if ( ! isset( $this->checklist_cache[ $post_id ] ) ) {
			...
			$test                              = $this->checklist_cache[ $post_id ];
			$new_test                          = preg_replace( '/(id="in-stores-(\d+)")/', '$1 ref="slpCategory-$2"', $test );
			$this->checklist_cache[ $post_id ] = $new_test;
	 ...
	 */
	public function getCategoryChecklistForVue() {
		if ( ! $this->cache['categoryChecklistForVue'] ) {
			...
			foreach ( $categories as $category ) {
				$categoryHTML .= sprintf(
					'<v-checkbox v-model="slpCategory-%d" id="slpCategory-%d" label="%s"></v-checkbox>',
					$category->term_id,
					$category->term_id,
					$category->name
				);
			...
			$this->cache['categoryChecklistForVue'] = $categoryHTML;
		}

		return $this->cache['categoryChecklistForVue'];
	}
	 ...
	 */
	private function map_categories_to_locations() {
		if ( ! $this->slplus->currentLocation->isvalid_ID() ) {
			return;
		}
		foreach ( $this->location_categories as $category ) {
			$this->add_RecordIfNeeded( $this->slplus->currentLocation->id, $category );
		}
	}
	...
	public function render_ExtraCategoryFields( $term_id = null, $category_data = null ) {
        ...
        <div id="slp_extended_data">
            ...
            <!-- Map Marker -->
			<?php
			$icon_maker = new SLP_Settings_icon( array(
				'name'  => 'map-marker',
				'label' => $this->slplus->Text->get_text_string( 'map_marker' ),
				'value' => $category_data['map-marker']
			) );
			$icon_maker->display();
			?>
			...
			$icon_maker = new SLP_Settings_icon( array(
				'name'  => 'medium-icon',
				'label' => $this->slplus->Text->get_text_string( 'medium_icon' ),
				'value' => $category_data['medium-icon']
			) );
			$icon_maker->display();
			?>
            ...
            <div class="input-group">
                <div class="form-field">
                    ...
                    <input type="text" id="category_url" name="category_url"
                           value="<?= $category_data['category_url'] ?>">
	 ...
	 *
	 * This is an array of location category IDs passed in from $_REQUEST['tax_input']['stores'].
	 * Only sets this if location_categories is empty.
	 */
	...
	public function convert_category_name_list_to_id_list( $category_name_list ) {
		...
		$category_list  = array();
		$category_slugs = explode( ',', $category_name_list );
	 ...
	 *
	 * @filters-called slp_location_page_attributes => set_page_taxonomy
	 */
	public function update_location_category() {
	 ...
	 * @return null
	 */
	private function add_RecordIfNeeded( $location_id = null, $category_id = null ) {
		if ( ! isset( $this->addon ) ) {
			return;
		}
		...
		$data = SLP_Power_Category_Data::get_instance();

		// Check if record exists
		//
		$matching_record = $data->get_Record( 'select_by_keyandcat', array(
			$location_id,
			$category_id
		) );
		...
		if (
			...
			$data->db->insert( $data->plugintable['name'],
				array(
					'sl_id'   => $location_id,
					'term_id' => $category_id
				)
