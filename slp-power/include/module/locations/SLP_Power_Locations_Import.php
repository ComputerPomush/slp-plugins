...
class SLP_Power_Locations_Import extends SLPlus_BaseClass_Object {
	const hook = 'slp_import_locations';        // "Manual" imports
	const csv_import_hook = 'cron_csv_import';  // Recurring (cron) imports
	...
	private $csv_clear_messages_on_import = true;
	...
	function initialize() {
		...
		$this->csv_clear_messages_on_import = $this->slplus->is_CheckTrue( $this->addon->options['csv_clear_messages_on_import'] );
		$this->default_mode                 = $this->addon->options['csv_duplicates_handling'];
	}
	 ...
	 * @return int|void
	 */
	public function start_remote_file_import( $params ) {
		wp_clear_scheduled_hook( self::csv_import_hook, array( 'import_csv', $this->file_meta ) );

		if ( empty( $params['url'] ) ) {
			return;
		}
		$this->get_remote_file( $params );
		$this->import( $this->attachment_id );
	}
	...
	public function get_scheduled_list() {
		$data      = array();
		$cron_list = $this->addon->get_all_crons_for_hook( self::csv_import_hook );
		foreach ( $cron_list as $meta ) {
			if ( ! empty( $meta['args'] ) ) {
				$data[ $meta['key'] ] = array(
					'key'       => $meta['key'],
					'timestamp' => $meta['timestamp'],
					'interval'  => ! empty( $meta['interval'] ) ? $meta['interval'] : '',
	 ...
	 *
	 * @param string|array $remote_file
	 */
	private function get_remote_file( $params ) {
		$remote_file = is_array( $params ) ? $params['url'] : $params;
		...
		$original_name = basename( $remote_file );
		...
		if ( is_wp_error( $tmp ) ) {
			...
			$this->slplus->notifications->add_notice( 'warning', __( 'Could not fetch the remote file.', 'slp-power' ) );
		...
		if ( is_wp_error( $this->attachment_id ) ) {
			@unlink( $file_array['tmp_name'] );
			$this->slplus->notifications->add_notice( 'warning', __( 'Could not sideload the remote file into media library.', 'slp-power' ) );
	...
	private function move_csv_to_slpdir() {
		...
		if ( ! file_exists( $this->file_meta['csvfile']['tmp_name'] ) ) {
			return null;
		}
		...
		if (
			...
			if ( ! rename( $this->file_meta['csvfile']['tmp_name'], $new_file ) ) {
				echo SLP_Admin_Helper::get_instance()->create_string_wp_setting_error_box(
	 ...
	 */
	private function set_options_from_meta() {
		if ( empty ( $this->file_meta['params'] ) ) {
			return;
		}
		$this->skip_geocoding               = $this->file_meta['params']['csv_skip_geocoding'];
		$this->load_data                    = $this->slplus->is_CheckTrue( $this->file_meta['params']['load_data'] );
		$this->csv_clear_messages_on_import = $this->slplus->is_CheckTrue( $this->file_meta['params']['csv_clear_messages_on_import'] );
		$this->default_mode                 = $this->file_meta['params']['csv_duplicates_handling'];
	}
	...
	private function open_file_and_start_importing() {
		...
		$this->file_meta = array_merge( array(
			'record'      => 0,
			'added'       => 0,
			'updated'     => 0,
			'not_updated' => 0
		), $this->file_meta );
		$this->set_options_from_meta();

		$this->processing_report   = array();
		$this->processing_report[] = ( sprintf( __( 'Starting CSV import - duplicates %s .', 'slp-power' ), $this->default_mode ) );
		...
		if ( $this->ok_to_process_file() ) {
			...
			while ( ( $this->data = fgetcsv( $this->filehandle, 0, ',', '"', '"' ) ) !== false ) {
				...
				$this->mode_for_current_record = $this->default_mode;  // Needed for Identifier special field processing.
	...
	public function prepare_for_import() {
		...
		add_filter( 'slp_csv_locationdata', array(
			SLP_Power_Category_Manager::get_instance(),
		...
		if ( $this->csv_clear_messages_on_import ) {
	 ...
	 *
	 * @used-by \SLP_Power_Locations_Import::prepare_for_import
	 * @used-by \SLP_Power_Locations_Import::schedule_remote_import
	 *
	 */
	private function set_FileMeta() {
		...
		// Cron Process Is Running... (import active?)
		//
	 ...
	 * @param string $hook
	 */
	public function stop_processing( $args, $hook = self::hook ) {
		if ( empty( $args ) ) {
			return;
		}
		$the_attachment_id = null;

		if ( ! is_array( $args ) ) {
			$the_attachment_id = $args;
			$args              = array( $the_attachment_id );
		}

		wp_clear_scheduled_hook( $hook, $args );
		if ( empty( $the_attachment_id ) ) {
			return;
		}
		...
		wp_update_attachment_metadata( $the_attachment_id, $this->file_meta );
