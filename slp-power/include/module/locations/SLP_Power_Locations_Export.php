...
class SLP_Power_Locations_Export extends SLPlus_BaseClass_Object {
	...
	/* @var array $active_columns */
	private $active_columns;

	/* @var string[] $dbFields The fields we want to export. */
	...
	public function add_tagalong_data_to_export( $locationArray ) {
		...
		$offset                         = 0;
		while ( $category = $this->addon->category_data->get_Record( array(
			'tagalong_selectall',
			'whereslid'
		), $locationArray['sl_id'], $offset ++ ) ) {
