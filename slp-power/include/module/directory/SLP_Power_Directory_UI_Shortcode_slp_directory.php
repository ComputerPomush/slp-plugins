...
class SLP_Power_Directory_UI_Shortcode_slp_directory extends SLPlus_BaseClass_Object {
	...
	private function createstring_List() {
		...
		$HTML = "<div class='slp_directory_list slp_directory_style_{$this->attributes['style']}' data-group_by='{$this->alternate_groupby_field}'>";

		$offset = 0;
		while ( $location = $this->slplus->database->get_Record( array(
			'select_for_slp_directory_by_group',
			...
			$HTML .=
				"<div class='slp_directory_entry slp_directory_entry_{$sanitized_property_data}' data-group_by_value='{$sanitized_property_data}'>" .
	...
	private function createstring_ListLink( $text, $field_name, $url_data = null, $url_field = null ) {
		...
		// __SELF__ locator page reference, links back to same page with JS loader.
		if ( $this->attributes['locator_page'] === '__SELF__' ) {
			$url = sprintf( '<a href="javascript:void(0)" class="slp_directory_link slp_directory_link_%s" onclick="mySLP.POWER.directory.show_locations_on_map( jQuery( this ).data( \'text_value\' ) )" data-text_value="%s">%s</a>', sanitize_title( $text ), $text, $text );
	...
	public function createstring_ShortcodeOutput() {
		...
		switch ( $this->attributes['style'] ) {
			...
			default:
				...
				if ( $this->attributes['locator_page'] === '__SELF__' ) {
					$directory_map_options = array(
						'hide_until_called'          => true,
						'immediately_show_locations' => '0',
						'hide_distance'              => '1',
						'hide_map'                   => '1',
						'hide_search_form'           => '1',
						'remove_credits'             => '1',
					...
					$HTML                  .= SLP_UI::get_instance()->render_shortcode( $directory_map_options, '' );
				}
