...
class SLP_Power_Pages_Admin extends SLPlus_BaseClass_Object {
	...
	function extend_location_edit() {
		...
		SLP_Admin_Locations::get_instance()->settings->add_ItemToGroup( array(
	...
	private function save_pages_settings() {
		if ( empty( $_REQUEST['page'] ) || ( $_REQUEST['page'] !== 'slp-pages' ) ) {
			return;
		}
	...
	private function set_page_actions() {
		...
		if ( $this->slplus->currentLocation->linked_postid > 0 ) {
			...
			$link = $this->create_string_action_entry( array(
				...
				'link'         => SLP_Admin_Locations::get_instance()->hangoverURL . '&act=createpage&sl_id=' . $this->slplus->currentLocation->id
			) );
			if ( ! empty( $link ) ) {
				$actions['create'] = $link;
			}
