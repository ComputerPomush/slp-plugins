...
class SLP_Power_Pages_UI_Shortcode_slp_pages extends SLPlus_BaseClass_Object {
    ...
    private function create_string_location_list( $output ) {
        ...
        if ( ! empty( $page_list ) && ! empty( $page_list->posts ) ) {
	        ...
	        $read_more_enabled = ! empty( $addon->options[ 'pages_read_more_text' ] );
            ...
            foreach ( $page_list->posts as $page ) {
                ...
                // Allow <!--more--> in description
	            $saved_description = '';
	            if ( $read_more_enabled && ! empty( $this->slplus->currentLocation->description ) ) {
                	$saved_description = $this->slplus->currentLocation->description;
		            $extended_description = get_extended( $this->slplus->currentLocation->description );
		            $this->slplus->currentLocation->description = $extended_description[ 'main' ];
	            }
                ...
                if ( $read_more_enabled ) {
                	if ( ! empty( $saved_description ) ) $this->slplus->currentLocation->description = $saved_description;
                    $extended_content = get_extended( $content );
                    $stripped_content = empty( $extended_content['extended'] ) ? '' : wp_strip_all_tags( $extended_content['extended'] , true );
	                $content = $extended_content['main'];
                    if ( ! empty( $stripped_content ) ) {
                        $content .=  sprintf( '<span class="slp_pages more"><a href="%s">%s</a></span>', get_permalink( $page->ID ), $addon->options[ 'pages_read_more_text' ] );
                    }
                }

	            $content = apply_filters( 'the_content' , $content );
	            $content = str_replace( ']]>', ']]&gt;', $content );
                $html .= sprintf( '<div id="slp_page_location_%d" class="%s">%s</div>', $location_id, $this->slplus->SmartOptions->pages_directory_entry_css_class, $content );
            }

        } else {
			if (  is_active_sidebar( 'slp-power-no-pages' ) ) {
				ob_start();
				dynamic_sidebar( 'slp-power-no-pages' );
				$html = ob_get_clean();
			}
        }
        return $html;
	 ...
	 * @param string $value
	 */
    private function set_meta_query( $value ) {
    	$regex = '/^(\w+)\W+(\w+)\W+(.+)$/';
    	preg_match( $regex, $value , $matches );
    	if ( count( $matches ) < 3 ) return;
    	$field = 'slp_location_' . $matches[1];
    ...
    private function set_page_retrieval_args() {
        if ( is_null( $this->page_retrieval_args ) ) {
            ...
            if ( ! empty( $this->meta_query ) ){
	            $this->page_retrieval_args['meta_query'] = array( 'relation' => 'AND' , $this->meta_query );
            }
