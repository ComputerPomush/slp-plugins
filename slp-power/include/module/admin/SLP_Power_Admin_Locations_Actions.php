...
class SLP_Power_Admin_Locations_Actions extends SLPlus_BaseClass_Object {
	...
	private function categorize_locations() {
		...
		//
		$inputCats = SLP_Power_Category_Manager::get_instance()->get_categories_from_input();
	...
	private function export_locations_to_local_csv_file() {
		...
		SLP_Power_Locations_Export::get_instance()->do_WriteFile( 'file://' . SLPLUS_UPLOADDIR . 'csv/exported_slp_locations.csv' );
	}
	...
	public function create_pages() {
		...
		if ( empty( $page_id ) ) {
			$page_id = new WP_Error( 'create_page_no_store_page', __( 'Store Page could not be created.', 'slp-power' ) );
		}
	...
	public function process( $action ) {
		...
		switch ( $action ) {
			...
			// Schedule Remote Locations Import
			//
			case 'import':
				$_REQUEST['selected_nav_element'] = '#wpcsl-option-import';
				SLP_Power_Cron::get_instance()->schedule_remote_file_retrieval();
				break;
	...
	private function tag_locations( $action ) {
		...
		while ( ! is_null( $id ) ) {
			...
			$original_tags = $this->slplus->currentLocation->tags;
			...
			if ( $original_tags !== $this->slplus->currentLocation->tags ) {
