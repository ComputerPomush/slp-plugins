...
class SLP_Power_Admin_Locations_Import extends SLP_Object_With_Objects {
	...
	private function configure_javascript_variables() {
		$manage_locations_vars = array(
			...
			'rest_geocode_url' => rest_url( SLP_REST_SLUG . '/v2/geocoding' ),
			'rest_imports_url' => rest_url( SLP_REST_SLUG . '/v2/imports' ),
			'upload_url'       => admin_url( 'async-upload.php' ),
		);

		wp_add_inline_script(
			'slppower_manage_locations',
			'const location_import = ' . wp_json_encode( $manage_locations_vars )
		);
	}
	...
	private function create_string_cron_schedule() {
		if ( ! isset( $this->file_meta ) ) {
			return '';
		}
		$html     = '';
		$schedule = wp_get_schedule( SLP_Power_Locations_Import::csv_import_hook, array(
			'import_csv',
			$this->file_meta
		) );
		if ( ! empty( $schedule ) ) {
	 ...
	 * @return string
	 */
	private function get_html_for_scheduled_import_list() {
		$cards             = array();
		$previous_key_card = array( 'schedule' => '', 'action' => '', 'timestamp' => '', 'url' => '' );
		$data              = SLP_Power_Locations_Import::get_instance()->get_scheduled_list();
		$listed_card_count = 0;
		foreach ( $data as $key => $meta ) {
			if ( $listed_card_count ++ > 10 ) {
				break;
			}

			// Clear all prior scheduled imports
			if ( isset( $meta['args'][1]['csvfile']['size'] ) ) {
				wp_clear_scheduled_hook( SLP_Power_Locations_Import::csv_import_hook, $meta['args'] );
				continue;
			}
			...
			$meta['duplicate'] = ( $previous_key_card === $this_key_card );

			if ( ! $meta['duplicate'] ) {
				$previous_key_card = $this_key_card;

				// Clean out duplicate import actions.
			} else {
				wp_clear_scheduled_hook( SLP_Power_Locations_Import::csv_import_hook, $meta['args'] );
			}
			$cards[] = $this->generate_recurring_import_card( $meta );
		}
		...
		return '<div class="v-application" id="recurring_imports"><div class="container container--fluid">' .
		       '<div class="row">' . __( 'Recurring cron jobs.', 'slp-power' ) . '</div>' .
	 ...
	 * @param $meta
	 */
	private function generate_recurring_import_card( $meta ) {
		$time = date( "Y-m-d\TH:i:s\Z", $meta['timestamp'] );

		$params = $meta['args'][1];

		$schedule = ucfirst( $meta['schedule'] );

		$action = $meta['action'];
		if ( $action === 'import_csv' ) {
			$action = __( 'Import CSV', 'slp-power' );
		}

		if ( ! empty( $params['url'] ) ) {
			$url_html = sprintf( '<div class="col-sm-12"><a href="%s" target="slp">%s</a></div>', $params['url'], $params['url'] );
		} else {
			$url_html = '';
		}
		...
		foreach ( $settings as $setting ) {
			$settings_html .= sprintf( '<div class="col-sm-12 pb-0 pt-0">%s: %s</div>', $setting, $this->slplus->is_CheckTrue( $this->addon->options[ $setting ] ) ? __( 'Yes', 'slp-power' ) : __( 'No', 'slp-power' ) );
		}

		$color = $meta['duplicate'] ? 'red lighten-4' : '';

		return <<<HTML
				<div id="{$meta['key']}" role='recurring_import_card' class="ma-1 pa-1 v-card v-card--outlined {$color} v-sheet v-sheet--tile theme--light">
					<div class="v-card__title">{$schedule} {$action}</div>
					<div class="v-card__subtitle">@{$time}</div>
...
HTML;
	...
	private function import_messages_group( $section_slug ) {
		if ( ! is_object( $this->addon->csvImporter ) ) {
			return;
		}
	...
	private function import_remote_file_group( $section_slug ) {
		...
		$this->settings->add_ItemToGroup( array(
			...
			'type'         => 'hyperbutton',
			'id'           => 'import_button',
			'button_label' => __( 'Schedule Location Import', 'slp-power' )
		) );
		...
		$this->settings->add_ItemToGroup( array(
			...
			'description'  => $this->create_string_cron_schedule() .
			                  $this->get_html_for_scheduled_import_list()
		) );
