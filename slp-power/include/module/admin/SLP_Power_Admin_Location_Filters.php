...
if ( ! class_exists( 'SLP_Power_Admin_Location_Filters' ) ) {
	...
	class SLP_Power_Admin_Location_Filters extends SLPlus_BaseClass_Object {
		 ...
		 * @used-by
		 *
		 * @uses \SLP_Power_Category_Manager::get_check_list
		 */
		...
		public function create_LocationSQLCommand( $request_data ) {
			...
			//
			if ( ! empty( $this->formdata['sl_id'] ) ) {
				add_filter( 'slp_ajaxsql_where', array( $this, 'filter_ExtendGetSQLWhere_LocationIn' ) );
				//$sqlParams[]  = join( ',' , $this->formdata[ 'sl_id' ] );
				$this->filter_active = true;
			}
		 ...
		 * @return string
		 */
		public function filter_ExtendGetSQLWhere_LocationIn( $where ) {
			$location_list = join( ',', $this->formdata['sl_id'] );

			return $this->slplus->database->extend_Where( $where, 'sl_id IN ( ' . $location_list . ' ) ' );
		}
	...
	if ( is_a( $slplus, 'SLPlus' ) ) {
		$slplus->add_object( new SLP_Power_Admin_Location_Filters( array( 'addon' => $slplus->AddOns->instances['slp-power'] ) ) );
	}
}
