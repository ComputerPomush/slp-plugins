...
class SLP_Power_Admin_Locations extends SLPlus_BaseClass_Object {
	...
	/* @var SLP_Power_Admin_Locations_Import $import_subtab */
			...
			$categorize_html = <<< SCRIPT
				<script type="text/javascript">
					...
					jQuery(document).ready( function(){
						jQuery('#slp_tagalong_fields input:checkbox').on('change',function(){
							var theCats="";
							var hiddenInputs=new Array();								
...
SCRIPT;
			$power_actions[] = array(
				'label'  => __( 'Categorize', 'slp-power' ),
				'value'  => 'categorize',
				'extras' => $categorize_html
			);
		}
	...
	public function modify_add_form() {
		...
		if ( $category_manager->get_category_count() > 0 ) {
			...
			$categoryChecklist = $category_manager->getCategoryChecklistForVue();
	...
	public function add_location_tab_panels( $settings ) {
		...
		if ( is_object( $this->import_subtab ) ) {
			$this->import_subtab->create_import_subtab( $this->settings );
		}
	}
