<#
.SYNOPSIS
    Scores suite-v022 against the v0.0.21 build and the v0.0.21 control, and
    AUDITS THE TAGS.
...
.DESCRIPTION
    ...
    A tag is a falsifiable claim about the control run:
      ...
      [v22]  MUST fail against the v0.0.21 blob. If it passes, the case does
             not discriminate and the tag is wrong.
      [both] MUST pass against the v0.0.21 blob. If it fails, either the tag
    ...
    the audit is clean, so do not pin it in Publish-Step19 before then.
...
.EXAMPLE
    .\test\Verify-Suite022.ps1
    .\test\Verify-Suite022.ps1 -ControlRef 5ae2d40
#>

[CmdletBinding()]
param(
    [string] $ControlRef       = 'v0.0.21',
    [string] $ControlMd5       = 'c8ef9f35c55f81c7f952dbf1e66e2aca',
    [string] $Suite            = 'test\suite-v022.php',
    [string] $Build            = 'build\out22\class.slp_avalon.php',
...
function Get-CaseTable {
    ...
    $t = @{}
    foreach ($l in $Lines) {
        # "  pass [v22]  A6  the disposal was recorded"
        if ($l -match '^\s{2}(pass|FAIL)\s+\[(v22|both)\]\s+(\S+)\s+(.*)$') {
...
Write-Host ''
Write-Host 'Verify-Suite022 r2  -  scoring and tag audit' -ForegroundColor Cyan
Write-Host ''
...
$ctl = Join-Path $env:TEMP 'ctl21.php'
...
if (-not $SkipControlPin -and $ctlHash -ne $ControlMd5) {
    ...
    Write-Host '    Either the ref is not v0.0.21, or the extraction altered bytes.'
...
foreach ($id in ($bTable.Keys | Sort-Object)) {
    ...
    $c = $cTable[$id]
    if ($b.Tag -eq 'v22'  -and $c.Result -eq 'pass') { $falseV21  += $c }
    if ($b.Tag -eq 'both' -and $c.Result -eq 'FAIL') { $falseBoth += $c }
}

if ($falseV21.Count) {
    Write-Host ''
    Write-Host '  [v22] cases that PASSED against the control - the tag is a false claim:' -ForegroundColor Red
...
if ($bScore -ne $nCases) {
    ...
    Write-Host ''
    Write-Host '  Pin this triple in Publish-Step19:'
